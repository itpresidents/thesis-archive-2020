...
export const getStudent = studentId => getApi().getStudent(studentId);
...
const realApi = {
  getAllStudents: () => realApi.getStudent(-1),
  getStudent: async studentId =>
    (await axios.get(`${baseApiUrl}?student_id=${studentId}`)).data
};

const fake2018Api = {
  getAllStudents: async () => (await axios.get("/2018/all.json")).data,
  getStudent: async studentId =>
    (await axios.get(`/2018/${studentId}.json`)).data
};
